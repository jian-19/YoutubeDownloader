...
class App:
    ...
    def download(self, song, playlist, audio, video, link):
        ...
        if audio:
            for conv_thread_data in self.conversion_threads:
                ...
                self.messgages('Convertendo', name[:42])  # Update the Label to show the song name we are converting
                thread.start()  # Start the Thread
                thread.join()  # Join the Thread so that the app will wait for the download to complete
    ...
    def messgages(self, m, song_name=None):
        if m == 'downloading':
            ...
            self.window.refresh()
        if m == 'converting':
            new_text = f"convertendo em mp3:\n{song_name[:35]}"
            self.window.Element('label').update(new_text, text_color='cyan')
            self.window.refresh()
        elif m == 'downloaded':
    ...
    def popups(type_popup, *args):
        if type_popup == 'invalid link':
            ...
            pg.popup(
                f'Download Concluido!',  font=("Arial", 15),
                any_key_closes=True,
                no_titlebar=True,
                background_color='white',
                text_color='black')
